...
export default async function handler(req, res) {
  ...
  if (!url || !key) {
    ...
    return res.status(400).json({ error: 'Missing SUPABASE_URL / SUPABASE_SERVICE_ROLE (or ANON) env vars.' })
  }
  const sb = createClient(url, key, { auth: { persistSession: false } })

  if (req.method === 'GET') {
    const { data, error } = await sb.from(table).select('payload').eq('id', 1).maybeSingle()
